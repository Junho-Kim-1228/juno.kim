...
export function DeveloperPage() {
  ...
  useEffect(() => {
    Promise.all([projectApi.list(), postApi.list()]).then(([projects, posts]) => setData({ projects: projects.slice(0, 4), posts: posts.slice(0, 4) })).catch(setError).finally(() => setLoading(false))
  }, [])
  ...
  return <>
    ...
    <section className="page-section"><div className="section-heading"><div><p className="eyebrow">기술 기록</p><h2>개발하며 남긴 메모</h2></div><Link to="/records">전체 보기</Link></div>{data.posts.length ? <div className="card-grid">{data.posts.map((post) => <PostCard key={post.id} post={post} />)}</div> : <p className="empty-row">아직 기록이 없습니다.</p>}</section>
  </>
}
