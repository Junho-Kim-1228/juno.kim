...
export function ProfilePage() {
  ...
  return <section className="form-page page-section">
    ...
    <form className="stack-form" onSubmit={submit}>
      ...
      <div className="form-row"><label>웹사이트<input type="url" value={profile.website_url} onChange={(event) => setProfile({ ...profile, website_url: event.target.value })} /></label><label>GitHub<input type="url" value={profile.github_url} onChange={(event) => setProfile({ ...profile, github_url: event.target.value })} /></label></div>
      <label>프로필 이미지<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setAvatar(event.target.files[0])} /></label>
